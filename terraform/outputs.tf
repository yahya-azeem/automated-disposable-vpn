...
output "instance_id" {
  ...
  value = var.local_mode ? docker_container.vpn_local[0].id : (
    var.active_region == "us-east-1" ? module.vpn_us_east_1[0].instance_id : (
...
output "public_ip" {
  ...
  value = var.local_mode ? "127.0.0.1" : (
    var.active_region == "us-east-1" ? module.vpn_us_east_1[0].public_ip : (
...
output "vpc_id" {
  ...
  value = var.local_mode ? "local-docker-vpc" : (
    var.active_region == "us-east-1" ? module.vpn_us_east_1[0].vpc_id : (
